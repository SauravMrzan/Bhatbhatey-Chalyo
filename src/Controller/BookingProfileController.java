...
public class BookingProfileController {
    private final BookingProfile b;
    private final BookDAO bookdao;
    
     public BookingProfileController(BookingProfile b) {
        this.b = b;
...
// Method to fetch data from the database, replace it with your actual implementation
  ...
  private class UpbtnListener implements java.awt.event.ActionListener {
    ...
    String register = b.getRegBox().getText().trim();
    String idcust = (String) b.getIDbox().getText().trim();
    ...
    BookingProfileModel u=new BookingProfileModel(book, register,idcust,datepick,datedrop);
 try {
        ...
        if (bookdao.isBookingExists(register, idcust)) {
            JOptionPane.showMessageDialog(b, "Bike " + register + " or Customer " + idcust + " is already booked", "Duplicate Booking", JOptionPane.WARNING_MESSAGE);
            return;
        }
        bookdao.addbook(u);
 ...
 private void UpbtnActionPerformed(java.awt.event.ActionEvent evt) {
    String book = b.getBookID().getText().trim();
    String register =  b.getRegBox().getText().trim();
    String idcust =  b.getIDbox().getText().trim();
    String datedrop = b.getDropDate().getText().trim();
    String datepick = b.getPickDate().getText().trim();
